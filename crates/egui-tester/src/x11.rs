...
impl X11Controller {
    ...
    pub fn capture(&self, window: &Window) -> Result<Frame> {
        ...
        let rgba = decode_x11_pixels(&image, visual.red_mask, visual.green_mask, visual.blue_mask);
        Ok(Frame::new(
...
fn decode_x11_pixels(image: &Image<'_>, red_mask: u32, green_mask: u32, blue_mask: u32) -> Vec<u8> {
...
mod tests {
    ...
    fn rgb888_fast_path_decodes_both_x11_byte_orders() {
        for (order, source) in [
            ...
            assert_eq!(
                decode_x11_pixels(&image, 0x00ff_0000, 0x0000_ff00, 0x0000_00ff),
